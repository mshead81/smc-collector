...
1. Pulls OHLCV candles from **Hyperliquid** (free, no key, not geoblocked) for each symbol/timeframe
...
- `SYMBOLS` — add/remove coins. `hlCoin` = Hyperliquid perp name (e.g. `BTC`, `SOL`, `HYPE`), `tvSymbol` = TradingView ticker for the image. **Verify each coin is listed on Hyperliquid** (app.hyperliquid.xyz) before relying on it — most majors + HYPE are; some newer alts may not be, and will show an error row until swapped.
